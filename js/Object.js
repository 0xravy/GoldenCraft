...
class Object {
    ...
    changePage(btn) {
        btn.onclick = () => {
            var bodyChild = document.body.children[1].children;
            ...
            secIds.map((id) => {
                if (btn.dataset.id == id) {
                    document.body.children[1].children[idNum].className =
                        "open";
                }
                idNum++;
            });
    ...
    addUser() {
        addUser.onclick = () => {
            ...
            if (inl === 0) return alert("يرجى كتابة اسم العميل 🫠");
            if (inl > 30)
                return alert(
            ...
            if (mood === "update") {
                if (inputChangeName.value == dataPro[moodIndex].name)
                    return alert("انت لم تقم بتغير الاسم 😐");
                for (let i = 0; i < dataPro.length; i++) {
                    thereName = dataPro[i].name == inputChangeName.value.trim();
                    if (thereName)
                        return alert("الاسم موجود يجرى وضع اسم اخر 😅");
                }
        ...
        if (addBtn == addInvoices) {
            addInvoices.onclick = () => {
                if (
                    ...
                    return alert("يجرى تعبئة جميع المدخلات 🧐");
                }
